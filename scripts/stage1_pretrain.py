"""Stage 1 pretrain driver: real video + actions on a LeRobot dataset.

Same losses, same architecture, same trainer as Stage 0 — only the data
source changes. That is the point: a stripped, general predictor that
runs the same recipe on toy and real data.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--resume", type=str, default=None,
                   help="path to a ckpt_step*.pt to load model+optimizer from")
    return p.parse_args()
...
def main() -> None:
    ...
    print(f"[data] building loader for {data_cfg.repo_id}", flush=True)
    ...
    print(f"[data] action_dim={a_dim}, image_size={image_size}, n_windows={len(loader.dataset)}", flush=True)
    ...
    print(f"[model] {n_params/1e6:.2f}M params ({n_train/1e6:.2f}M trainable)", flush=True)
    ...
    scaler = None  # bf16 doesn't need a grad scaler
    ...
    if args.resume:
        ...
        try:
            ...
            print(f"[ckpt] optimizer state mismatch (ok if param-groups differ): {e}", flush=True)
